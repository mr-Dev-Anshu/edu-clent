...
import {
  PageHeader, MetricCard, DataTable, FilterBar,
  AvatarCircle, ConfirmDialog,
...
import { TableColumn, FilterField } from '@/types'
...
export function ClassesPage() {
  ...
  const { data: classes = [], isLoading } = classesHook.useData()
  const { mutateAsync: deleteClass }      = classesHook.useRemove()

  const {
    paginated, total, page, pageSize, query, filters, sort,
    setPage, handleSearch, handleFilter, handleSort, resetFilters,
  ...
  const handleEdit = (classData: ClassRow) => {
    ...
    console.log('Edit clicked:', classData.name)
    // TODO: setEditOpen(true) when Member 2's EditClassModal is ready
  }

  const handleManage = (classData: ClassRow) => {
    setSelected(classData)
    console.log('Manage Sections clicked:', classData.name)
    // TODO: setSectionsOpen(true) when Member 2's ManageSectionsModal is ready
  }
      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            <button
              onClick={() => console.log('Add New Class — drawer coming in Member 2')}
