            ...
            <React.Fragment key={step.id}>
              <div className="flex flex-col items-center gap-2">
                <div className={cn(
                  ...
                  activeStep >= idx ? "bg-slate-900 border-slate-900 text-white" : "bg-white border-slate-200 text-slate-400"
                )}>
                  {activeStep > idx ? <Check size={14} /> : idx + 1}
                </div>
                <span className={cn("text-[9px] font-bold uppercase tracking-widest", activeStep >= idx ? "text-slate-800" : "text-slate-400")}>
                  {step.title}
                </span>
              </div>
              {idx < steps.length - 1 && (
                <div className={cn("h-[1px] flex-1 mx-4", activeStep > idx ? "bg-slate-900" : "bg-slate-100")} />
              )}
            </React.Fragment>
          ))}
        </div>
                ...
                .map((section) => (
                  <div key={section.id} className="mb-10">
                    <div className="flex flex-col mb-6 border-l-4 border-indigo-500 pl-4">
                      <h2 className="text-[13px] font-black text-slate-800 uppercase tracking-wider">{section.sectionTitle}</h2>
                    ...
                    <div className="grid grid-cols-12 gap-x-5 gap-y-4">
                      {section.fields.map(field => <FormField key={field.id} field={field} />)}
                    </div>
                  </div>
                ))}
            </motion.div>
